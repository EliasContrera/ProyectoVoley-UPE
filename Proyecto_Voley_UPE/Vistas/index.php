...
if ($stmt) {
        $noticiasPrincipales = $stmt->fetchAll(PDO::FETCH_ASSOC); //Esta variable la utilizo para las imagenes del carrousel
        $stmt->closeCursor();
}
    

$sql = "SELECT id, titulo, descripcion, pathFoto FROM NoticiasPrincipales"; //Consulta para las noticias debajo del carrousel
$stmt = $conexionPDO->query($sql);
